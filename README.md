...
A local `chatterbox-tts` service (Resemble AI's `ResembleAI/chatterbox-turbo` via `chatterbox-tts`) is also bundled; select the Chatterbox Turbo provider in the UI or set `TTS_PROVIDER=chatterbox_local`. Chatterbox clones a voice from a reference clip, so each bundled voice (`female_warm`, `male_calm`, `female_clear`, `male_clear`) maps to a reference wav fetched from the public demo bucket on first use; override the map with `CHATTERBOX_TTS_VOICE_REFS` (JSON `{"id":"url_or_/models/path.wav"}`). It exposes the same per-page Reroll button plus an Advanced TTS parameters panel (temperature, `top_p`, `top_k`, `repetition_penalty`, `norm_loudness`, and pause/chunk/trim tuning; the Turbo checkpoint ignores `cfg_weight`/`exaggeration`/`min_p`, so those are intentionally omitted). Paralinguistic tags like `[laugh]` and `[cough]` can be written inline in a transcript.
